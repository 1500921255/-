<template>
  <div class="app-container">
    ...
    <div class="filter-container" style="margin: 10px 0 10px 0;">
      <el-input @keyup.enter.native="handleFind" class="filter-item" clearable placeholder="请输入用户名"
        style="width: 200px;" v-model="keyword"></el-input>

      <el-select clearable placeholder="账号类型" style="width:140px" v-model="accountSourceKeyword">
        <el-option :key="item.uid" :label="item.dictLabel" :value="item.dictValue"
          v-for="item in accountSourceDictList"></el-option>
      </el-select>

      <el-select clearable placeholder="评论状态" style="width:140px" v-model="commentStatusKeyword">
        <el-option :key="item.uid" :label="item.dictLabel" :value="item.dictValue"
          v-for="item in commentStatusDictList"></el-option>
      </el-select>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column align="center" label="头像" width="120">
        <template slot-scope="scope">
          <img
            :src="scope.row.photoUrl ? scope.row.photoUrl:'https://wpimg.wallstcn.com/f778738c-e4f8-4870-b634-56703b4acafe.gif'"
    ...
    <el-dialog :title="title" :visible.sync="dialogFormVisible">
      <el-form :model="form" :rules="rules" ref="form">
        ...
        <el-row :gutter="24">
          ...
          <el-col :span="9">
            <el-form-item label="性别" :label-width="formLabelWidth" prop="gender">
              <el-radio :key="gender.uid" :label="gender.dictValue" border size="medium"
                v-for="gender in genderDictList" v-model="form.gender">{{gender.dictLabel}}
              </el-radio>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="24">
          <el-col :span="9">
            <el-form-item label="状态" :label-width="formLabelWidth" prop="status">
              <el-select placeholder="请选择" style="width:205px" v-model="form.status">
                <el-option :key="item.uid" :label="item.dictLabel" :value="parseInt(item.dictValue)"
                  v-for="item in accountStatus"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
          ...
          <el-col :span="9">
            <el-form-item label="用户标签" :label-width="formLabelWidth" prop="userTag">
              <el-select placeholder="请选择" style="width:205px" v-model="form.userTag">
                <el-option :key="item.uid" :label="item.dictLabel" :value="parseInt(item.dictValue)"
                  v-for="item in userTagDictList"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-form-item :label-width="formLabelWidth" label="简介">
          <el-input :autosize="{ minRows: 3, maxRows: 10}" placeholder="请输入内容" style="width: 70%" type="textarea"
    ...
    <avatar-cropper :height="300" :key="imagecropperKey" :url="url" :width="300" @close="close"
      @crop-upload-success="cropSuccess" lang-type="zh" v-show="imagecropperShow" />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getDictList: function () {
      var dictTypeList = ['sys_account_source', 'sys_comment_status', 'sys_user_sex', 'sys_user_tag', 'sys_account_status']
      getListByDictTypeList(dictTypeList).then(response => {
        if (response.code == this.$ECode.SUCCESS) {
          ...
          this.accountStatus = dictMap.sys_account_status.list
        }
      });
    },
